...
```
客户端:21  ─── 控制连接 ───→ 服务器:21
客户端:5000 ─── 数据连接 ←─── 服务器:20

客户端告知服务器自己的 IP + 端口，服务器主动发起数据连接。
```

| 时机 | 操作 |
|------|------|
| 客户端 | 发送 PORT 命令，告知自己的 IP 和端口（格式：h1,h2,h3,h4,p1,p2）|
| 服务器 | 解析 PORT 参数，主动 connect 到客户端指定的地址和端口 |
| 数据 | 通过建立的数据连接传输文件或列表 |

**适用场景**：服务端和客户端在同一内网，或者客户端有公网 IP。如果客户端在 NAT/防火墙后面，服务器无法主动连入，会失败。

#### 被动模式（PASV）

```
客户端:21  ─── 控制连接 ───→ 服务器:21
客户端:5001 ─── 数据连接 ───→ 服务器:30001

服务器开放一个临时端口，客户端主动连接过来。
```

| 时机 | 操作 |
|------|------|
| 服务器 | 在数据端口范围内（默认 30000-30010）创建监听 socket |
| 服务器 | 响应 PASV 命令，返回 IP 和端口给客户端 |
| 客户端 | 主动 connect 到服务器指定的地址和端口 |
| 数据 | 通过建立的数据连接传输文件或列表 |

**适用场景**：互联网传输（客户端只需要能主动发起连接，无需公网 IP），也是现代 FTP 客户端默认使用的模式。
...
| **防火墙友好** | ❌ 客户端需公网 IP | ✅ 客户端只需能外发连接 |
| **服务端配置** | 简单，无需开放额外端口 | 需要开放一段数据端口范围 |
| **NAT 穿透** | ❌ 基本不可用 | ✅ 可用 |
| **典型场景** | 内网 FTP 传输 | 互联网 FTP 传输 |
